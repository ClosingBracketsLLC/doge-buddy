...
Design docs: `docs/superpowers/specs/`. **Phase 5 (sourcing agent) is live:** weekly runs harvest trends, fetch suppliers, submit winning proposals, apply approvals → live-listed products on the store. **Phase 6A (support email plumbing) is live too:** Gmail ingest, triage, the admin ticket surface, and escalation alerts are all verified in production. **Phase 6B (the support agent) is live on the email path** — per-ticket Agent SDK sessions with a Postgres-backed transcript, read-only ticket/order tools, drafted replies that only ever go out behind an owner approval, and the apply workers that send the threaded reply / issue the Shopify refund; **Tier-2 signed off 2026-08-30** (Gmail + Outlook threading, the send-recovery marker, session resume across a redeploy, the reject-with-reason re-draft loop, a live Shopify refund with the durable double-refund guard exercised, and the Gmail fixtures re-recorded against the real mailbox). The only carry-over is the CJ dispute write, which needs a real supplier order and rides on the canary. The store runs a **no-refund policy** (all sales final for change of mind; inspected-return replacements for damage) enforced end to end: refunds hard-locked to manual approval, an owner-editable guidance layer at `/admin/guidance`, and a validator that screens unbacked promises — coupons included. A Turnstile-gated `/contact` form feeds the same pipeline — a fixed-copy acknowledgement from support@ creates the Gmail thread the agent's reply then threads onto (live-walked 2026-08-31; Outlook.com drops first-contact mail from the new domain until reputation builds — tracked on the checklist). **Phase 7 (product scoring) is built:** nightly keep/watch/deprecate scoring that observes from launch, plus a revenue-gated weekly deprecation digest whose flagged underperformers only ever act — draft + unpublish + CJ webhook teardown — behind an owner approval (manual mode by default). **Catalog P0 is built (2026-08-31, branch `catalog-p0`):** a single `CATEGORIES` source of truth backs four real Shopify collections, newly listed and backfilled products get human URLs/tags/productType/SEO plus tracked inventory sourced from CJ's stock, and a 6-hourly `inventory.sync` job keeps that quantity current — live tier (running it against the real store) is still pending. Next: the Phase 7 canary launch — pre-publish anti-spam hardening on the code side, then the owner-side launch items (tracked on `docs/OWNER-CHECKLIST.md`). Ops runs deployed on Railway with Shopify+CJ webhooks live end-to-end; Gmail support mail is polling-based (no Pub/Sub, per the parent spec); storefront deploys to Oxygen on push. Owner tasks + the next-session pointer live in `docs/OWNER-CHECKLIST.md`.
