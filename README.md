...
Design docs: `docs/superpowers/specs/`. **Phase 5 (sourcing agent) is live:** weekly runs harvest trends, fetch suppliers, submit winning proposals, apply approvals → live-listed products on the store. **Phase 6A (support email plumbing) is live too:** Gmail ingest, triage, the admin ticket surface, and escalation alerts are all verified in production. **Phase 6B (the support agent) is live on the email path** — per-ticket Agent SDK sessions with a Postgres-backed transcript, read-only ticket/order tools, drafted replies that only ever go out behind an owner approval, and the apply workers that send the threaded reply / issue the Shopify refund; production-verified 2026-08-30 (Gmail + Outlook threading, the send-recovery marker, session resume across a redeploy, the reject-with-reason re-draft loop). The refund/money path is the one unverified remainder. The store runs a **no-refund policy** (all sales final for change of mind; inspected-return replacements for damage) enforced end to end: refunds hard-locked to manual approval, an owner-editable guidance layer at `/admin/guidance`, and a validator that screens unbacked promises — coupons included. **Phase 7 (product scoring) is built:** nightly keep/watch/deprecate scoring that observes from launch, plus a revenue-gated weekly deprecation digest whose flagged underperformers only ever act — draft + unpublish + CJ webhook teardown — behind an owner approval (manual mode by default). Owner-side next: a test-gateway order, then the refund/CJ-dispute Tier-2 walks and the Phase 7 canary-launch items (tracked on `docs/OWNER-CHECKLIST.md`). Ops runs deployed on Railway with Shopify+CJ webhooks live end-to-end; Gmail support mail is polling-based (no Pub/Sub, per the parent spec); storefront deploys to Oxygen on push. Owner tasks + the next-session pointer live in `docs/OWNER-CHECKLIST.md`.
