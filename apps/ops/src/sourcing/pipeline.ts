...
export interface SourcingPipelineDeps {
  ...
  notify: NotifyOwner
  adminBaseUrl?: string
  /** null = SERPAPI_KEY absent at boot → the trends stage is skipped (spec §Stage 2). */
...
export async function runSourcingPipeline(deps: SourcingPipelineDeps): Promise<SourcingPipelineResult> {
  const { db, adapter, settings, alert, enqueue, notify, adminBaseUrl, trends, queryFn, force } = deps
