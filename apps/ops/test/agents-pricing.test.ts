...
describe('agents/pricing', () => {
  ...
  describe('createUsageAccumulator', () => {
    ...
    it('buckets model-less usage under "unknown" key at sonnet rates', () => {
      const acc = createUsageAccumulator()
      ...
      // Should tally under 'unknown' key
      expect(tally.perModel['unknown']).toEqual({
        inputTokens: 1000,
        outputTokens: 500,
      ...
      // Cost at sonnet rates: (1000×3 + 500×15 + 2000×0.3) / 1e6
      // = (3000 + 7500 + 600) / 1e6
      // = 11100 / 1e6
      // = 0.0111
      expect(tally.estimatedCostUsd).toBeCloseTo(0.0111, 6)
    })
  })
})
