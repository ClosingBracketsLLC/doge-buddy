...
- [ ] 🟡 **`git push origin main`** — docs-only commits (Tier-2 walk closures + the refreshed next-session pointer). No deploy urgency; push so the history is safe off-machine.
