...
- [ ] 🔴 **Paste this into `/admin/guidance`** (edit freely — it's yours; the hard rules still win
  ...
  > items: ask for a photo of what they received, tell them we'll email return instructions, and
  > that a replacement ships at no charge once their return is in transit (a return IS required);
  > do not offer a refund unless the owner has said a replacement isn't possible. For
