...
**Next build session starts here →** **Sourcing upgrade 1 (market-price tool + 1.3× gate): **(1) BUILT 2026-09-01 (branch `sourcing-market-price`)** — live check pending (one `run-sourcing --max-winners 2`: SerpApi requests ≤ 25, a proposal summary carrying `market $… median ×…`, a `market_price` row in `sourcing_signals` with offerCount ≥ 5). (2) is next to spec. Owner items it needs before its live tier: `SERPAPI_KEY` present on the Railway ops service, and the SerpApi plan's monthly search quota (design assumed 250; if 100, lower the per-run cap to 15).** Still queued behind it: runway **B14** (`seed-collections` → `backfill-listings --dry-run` then real →
