...
**Next build session starts here →** **The Phase 7 canary launch.** Tier-2 is **signed off** as of 2026-08-30 late evening: #1/#1a (email → approve → threaded reply in Gmail AND Outlook, marker round-trip), #2 (live Shopify refund `999129448536` on order #1001, note round-trip verbatim, second delivery skipped, crash-window re-entry recovered), #3 (session resume across a redeploy), the reject-with-reason re-draft loop and the no-refund policy stack all live, and the `GMAIL_CONTRACT=1` re-record done (`e0170eb`). Three real bugs were caught and fixed on the way (per-transaction `orderId`, gateway-null guard, `@idempotent` on the mutation FIELD) plus the cross-thread reply fix. **Code-side opener:** the pre-publish anti-spam hardening from "Publish the storefront" (the pre-triage spam short-circuit) — Claude builds it before the storefront goes public. Then the canary (parent spec §(c)5): CJ wallet top-up, POLICY_COPY → Shopify Settings, Shopify Payments + US tax, store-transfer re-verification, publish the storefront (owner-gated; do the pre-publish anti-spam hardening first — see "Publish the storefront"), swap off the test gateway → first REAL order end-to-end, which also closes **#4** (`openCjDispute` needs a placed supplier order) and live-validates scoring's order→variant join. Also open: the DB-password rotation (two chat exposures now). Tools left behind this session: `inspect-mailbox`, `seed-refund-proposal`, `read-refund-state`, `redeliver-apply` (all under `pnpm --filter @doge-buddy/ops …`); Claude's raw writes to the Railway DB are classifier-blocked — hand Robert the SQL. Test-address rule: repeat-complainant counts tickets, so from any test address only ever reply on existing threads.
