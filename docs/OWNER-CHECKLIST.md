...
- [ ] 🔴 **BLOCKING before ANY CN product goes on sale — CJ's duty/DDP answers.** This is now the
  ONLY thing gating the cheap lane; the code half is built (see below). **Get CJ's written answers**
  to (a) is the line DDP, so the customer is never billed on delivery? (b) does the quoted freight
  include duty? (c) who is the declared Importer of Record? Then confirm it empirically on the
  canary — place that order through a CN product. The shipping policy now says in writing "no
  customs charges or extra fees on delivery", so a surprise duty bill on a customer's doorstep is a
  broken promise, not just an unexpected cost.
- [x] ✅ **Origin-aware fulfillment — BUILT 2026-09-07.** The order path no longer assumes US
  anywhere. Listing records each variant's warehouse and the delivery window its buyer was shown;
  inventory sync reads stock from that warehouse (without this a CN product syncs to 0 and shows
  "sold out"); the planner checks stock, freight and the promised window per warehouse; a cart that
  mixes origins becomes **one supplier order per warehouse**, each placed with CJ from its own
  origin and tracked as its own Shopify fulfillment; and the overdue sweep measures each leg
  against its own promise instead of a site-wide 7 days. A leg that can't ship parks for you while
  its sibling still goes out. Spec `superpowers/specs/2026-09-06-origin-aware-fulfillment-design.md`,
  plan `superpowers/plans/2026-09-06-origin-aware-fulfillment.md`.
- [ ] 🔴 **DEPLOY ORDER — migrations 0013 and 0014 BEFORE the code that reads them.** Same strict
  order as 0008/0009: the new code writes `supplier_orders.warehouse_country`,
  `supplier_orders.promised_max_days` and `supplier_variant_mappings.delivery_max_days` on every
  order it touches, so a redeploy before the migration throws on every place-order job while
  `/healthz` stays green (a silent fulfillment outage). (1) From this checkout:
  `DATABASE_URL='<Railway PUBLIC postgres url>' pnpm --filter @doge-buddy/db migrate` → expect
  `migrations applied` (14/14). (2) Then `git push origin main` and let Railway redeploy.
...
- [ ] ⚪ **Product page v2 LIVE CHECK** (after `product-page-v2` is merged to `main` and pushed; branch built 2026-09-01 — all 17 plan tasks committed, code+tests green, typecheck clean). Two runs, both from **inside the Railway ops service** (same localhost-DB trap as B14/the sourcing check above — a local run writes to the dev DB, not the live one):
  ...
  1. ✅ **DONE 2026-09-02** — `backfill-listings --dry-run` then real, from the Railway shell. **v2 pass: 23 products, 30 variant images attached (all READY), 9 supplier_reviews metafields written, 0 failures** — live-verifies `productUpdate(product, media)` create+attach, `productVariantAppendMedia`, `metafieldsSet`, `metafieldDefinitionCreate` (Claude's read-only probe confirmed all four `dogebuddy` v2 definitions with `storefront=PUBLIC_READ`, distinct per-color media on the 3-variant leash, and real parsed reviews e.g. `average=5 count=1`), and the `product/productComments` wire shape (cj-api-notes updated; 14 products had no usable/rated reviews — info-alert degrade, by design). `fileDelete`-of-media was never exercised (zero media failures) — stays a fixture assumption, non-fatal path. Rerun 2026-09-02 proved idempotency (v2 pass skipped all 30 already-imaged variants, 0 media created, 0 failures) but the pass-1 stock-read 429s ROVE — each run's ~90-call burst trips CJ's rate limit on a few random reads (two originals cleared, three new ones failed, one repeated). **No further reruns needed**: the affected variants' catalog fields are done, and `inventory.sync` (6h cron, per-variant retries) corrects their stock numbers on its own; rerunning just whack-a-moles and re-burns ~500 CJ points/run on review re-fetches. BOTH FOLLOW-UPS BUILT 2026-09-02 (on local main, land with your next push + Railway deploy): the CJ client's 429 backoff is now 5 attempts / 1s base — up to 15s of patience per call (`a00f6c7`), which should end the roving stock-read failures on future backfills; and the aggregate line says "1 marketplace rating" singular (`666ab7f`). Still open: (b) points cost of productComments — CJ dashboard when convenient; (c) CJ reviews arrive in the buyer's language (Spanish seen live) — judge on the Fold eyeball and tell Claude ONLY if you want a language filter (deliberately not built: it would erase most of the 9 live review sections).

  2. ✅ **DONE 2026-09-03** — first fully-v2 listing is LIVE end-to-end: `foldable-portable-dog-water-bottle-…-a6df7732` (ACTIVE; 4 media all READY; BOTH variants carry their own image; highlights/specs/whats_in_box written; supplier_reviews absent by design — CJ had none usable). Took three live bug-hunts to get there, all fixed on main: the scrubber's "secure"⊃"cure " false positive (`40da0b7`), and the per-variant `file` LINKING semantics — `files` must contain every variant image URL or productSet rejects with "File original source missing from the product files input" (`6901802`; proposal a6df7732 dead-lettered, was recovered via status flip + `redeliver-apply`). Agent priced freight-aware on its own ($23.99/$28.99 clearing floor + 1.3× ceiling). Also learned: `workflow.sourcing.mode` was AUTO — that's why no Telegram approve message ever came (auto = no notify, straight to apply). **Remaining from this item:** flip `workflow.sourcing.mode` back to `manual` on `/admin/settings` (recommended until launch); the sourcing_signals `offerCount ≥ 5` SQL check from the market-price item above; and the media-order observation (does the variant image or the first gallery image drive `featuredImage`) during the Fold eyeball.
