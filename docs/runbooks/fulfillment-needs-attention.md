...
fulfillment pipeline for a human to look at — `fulfillment.place-order`, `fulfillment.pay-order`,
and `fulfillment.reconcile`'s sweeps all deliberately stop touching it once it lands here
(`executePlaceOrder`'s resume switch: `case 'needs_attention': ... return`). One deliberate
exception: a **CJ ORDER webhook reporting CANCELLED** on a parked row is a legal direct
transition (`needs_attention -> cancelled`) and is applied automatically — so a parked row can
disappear into terminal `cancelled` without operator action when CJ cancels the order on their
end. Otherwise it stays parked until an operator moves it, by hand, to one of three places:
...
There is no admin UI for this yet (planned for Phase 4) — recovery today means a direct
`UPDATE supplier_orders SET status = '<target>' WHERE id = '<row id>'`, **always** followed by
re-sending `fulfillment.place-order { orderGid }` (singleton-keyed on the order gid). The re-send
is not an optional speed-up: no reconcile sweep re-enqueues work for an existing
`supplier_orders` row (sweep 1 handles only orders with no row at all; sweep 3 only applies CJ
status/tracking drift to already-active rows), so a recovered row left without a re-send sits
inert until sweep 4 re-parks it as `overdue`.
...
adapter pre-checks `shopping/order/list` by that same key (filter param `orderNumbers`, matched
client-side on each entry's `orderNum` — see `docs/cj-api-notes.md` §Order identity) before ever
calling create, so a resumed row can't double-place with the supplier either. Re-placement is resumable by
...
| --- | --- | --- |
| `unmapped_item` | No *usable* mapping for a line item's variant — either no `supplier_variant_mappings` row for this supplier, or the row exists but the joined `product_variants.supplier_cost_cents` (or `shopify_variant_gid`) is NULL, which `loadMappings` deliberately drops. | Add the mapping and/or set the missing supplier cost / variant gid, then `needs_attention -> pending`. |
...
| `pay_order_failed` | The `fulfillment.pay-order` job threw a non-balance error on all 5 pg-boss attempts; the dead-letter hook parked the row from `confirmed`/`awaiting_funds` with the underlying message after the prefix. | Read the message, verify the order's actual state with CJ (`getOrderDetail`), then `-> confirmed` to retry payment or `-> cancelled` if the supplier-side order is dead. |
...
- **`needs_attention -> confirmed`**: skips replanning and gate re-checks entirely — jumps straight
  ...
  its total is acceptable) and all that's left is payment. (CJ rows carry `shipment_order_id: NULL`
  by design — createOrderV3 never returns one; the pay executor falls back to `supplier_order_id`,
  so a null shipment id alone is not a reason to avoid this path.)
